...
/** Página para que el estudiante radique el informe final (PDF o URL) */
export default function SubmitInformeFinal() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await supabase.from("audit_events").insert({
        ...
        user_id: user.id,
        event_type: "INFORME_FINAL_SUBMITTED",
        description: `Informe final radicado (versión ${version})`,
        metadata: { version, submission_type: submissionType },
      });

      toast({ title: "Informe final radicado exitosamente" });
      navigate("/dashboard");
    } catch (error: any) {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <Card>
        <CardHeader>
          <CardTitle>Radicar Informe Final</CardTitle>
          <CardDescription>Sube tu documento de informe final o proporciona un enlace externo.</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-3 pt-2">
              <Button type="submit" disabled={submitting}>{submitting ? "Radicando..." : "Radicar Informe Final"}</Button>
